...
DEFAULT_ROBOT_TYPE: Final[str] = os.getenv("SO_ARM_ROBOT_TYPE", "so101")
DEFAULT_SERIAL_PORT: Final[str] = os.getenv("SO_ARM_PORT", "/dev/cu.usbmodemXXXXXXXX")
DEFAULT_REMOTE_IP: Final[str] = os.getenv("SO_ARM_REMOTE_IP", "192.168.1.1")
...
class RobotConfig:
    ...
    lerobot_config: Dict[str, Any] = field(
        default_factory=lambda: {
            ...
            "cameras": {
                # Override the index with SO_ARM_CAMERA_INDEX for your computer.
                "front": OpenCVCameraConfig(
                    index_or_path=int(os.getenv("SO_ARM_CAMERA_INDEX", "0")),
                    fps=DEFAULT_CAMERA_FPS,
                    width=DEFAULT_CAMERA_WIDTH,
                    height=DEFAULT_CAMERA_HEIGHT,
                ),
                # iPhone Continuity Camera (index 2) は必要になったら下記を有効化
...
Robot has 1 camera:
- front: Logitech C270 at the base, looks forward (main camera).
  When objects are inside the gripper they may be close to the lens and occupy most of the view.
